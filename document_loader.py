...
# FIX: Safer Mistral client initialization with fallback
client = None
try:
    ...
    try:
        ...
        print("⚠️ Mistral client not initialized — OCR unavailable")
...
def ocr_image_with_mistral(image_path: str) -> str:
    ...
    with open(image_path, "rb") as f:
        response = client.ocr(model="mistral-ocr-latest", file=f)
...
def pdf_to_images(pdf_path: str) -> list:
    ...
    for page_index in range(len(doc)):
        ...
        pix      = page.get_pixmap()
        img_path = f"/tmp/temp_page_{page_index}.png"
        pix.save(img_path)
